...
class SlotPredictor:
    ...
    black_box_theme = attr.Factory(create_black_theme)
    green_text_theme = attr.Factory(create_green_text_theme)
    white_text_theme = attr.Factory(create_white_text_theme)
    ...
    def refresh_payouts_panel(self, position_estimate: PositionEstimationResult):
        ...
        if position_estimate.position > 0:
            for i in range(self.payout_page_size):
                ...
                if payout_index < MAX_REELS_IN_GAME:
                    dpg.show_item(f"payout_row_{i}")

                    payout = self.context.get_payout_estimate(payout_index - i, iterations=i + 1)
                    payout_row_text = f"{payout_index}: {payout}"

                    # Change text color to green if there is a special bonus
                    if self.context.check_bonus(payout_index - i + 1):
                        payout_row_text += "  [BONUS]"
                    #     dpg.bind_item_theme(f"payout_row_{i}", self.green_text_theme)
                    # # else:
                    # #     dpg.bind_item_theme(f"payout_row_{i}", self.white_text_theme)
    ...
    def on_bet_click(self, bet: int):
        ...
        self.context.current_bet = bet
        self.set_context(self.context)

    def build_ui(self):
        # self.black_box_theme = create_black_theme()
        # self.white_text_theme = create_white_text_theme()
        # self.green_text_theme = create_green_text_theme()
                    ...
                    with dpg.group(horizontal=False, tag="CenterPanel"):
                        with dpg.child_window(width=390, height=258):
                            ...
                            with dpg.child_window(tag="GridBox", border=True, autosize_x=False, autosize_y=False):
                                dpg.bind_item_theme("GridBox", self.black_box_theme)
